...
switch ( $request ) {
    ...
    case 'witzuli':
        ...
        $api_id = (urldecode($_POST['api_id']));
        $api_key = (urldecode($_POST['api_key']));
        $translation_engine = (urldecode($_POST['translation_engine']));
        $language_pair = (urldecode($_POST['language_pair']));
        $content_type = (urldecode($_POST['content_type']));
        //FOR PROXY CASE
        //$text = html_entity_decode(str_replace("\/","/",str_replace("\'","'",urldecode($_POST['text']))));
        $text = htmlspecialchars_decode(stripcslashes($_POST['text']));
        //ELSE
        //$text = htmlspecialchars_decode(stripcslashes($_POST['text']));
        //$translation_type = (urldecode($_POST['translation_type']));
		//error_log("TRANSLATION TYPE");
		//error_log($translation_type);
		
		if( $translation_type == 'standar' ){
	        $body = array(
			    'api_id'    => $api_id,
			    'api_key'   => $api_key,
		...
		//Try again with raw text in text mode
		elseif ($translation_type == 'clean') {
...
switch ( $request ) {
	...
	//ELHUYAR edition.
    case 'witz_icl_witzultzailea_eus':
        $iclsettings['api_id'] = ($_POST['api_id']);
        $iclsettings['api_key'] = ($_POST['api_key']);
        $this->save_settings($iclsettings);
        //echo '1|';
        $out = __( 'Setting correctly saved.', 'sitepress' );
        echo '1|' . $out;
        break;

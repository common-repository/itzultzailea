...
function iclSaveForm() {
	...
	//ELHUYAR edition.
    jQuery('form[name="' + form_name + '"] .wpml-form-message').html('').hide();
	...
	jQuery.ajax({
		...
		success: function (msg) {
			...
			if (parseInt(spl[0]) == 1) {
				...
				//ELHUYAR edition.
                if (form_name == 'witz_icl_witzultzailea_eus'){
                    var witz_icl_message_form = jQuery('form[name="' + form_name + '"] .wpml-form-message');
                    witz_icl_message_form.html('<p style="color:#008000">'+spl[1]+'</p>');
                    witz_icl_message_form.fadeIn();
                }
                ...
                if (form_name == 'witz_icl_witzultzailea_eus'){
                    var witz_icl_message_form = jQuery('form[name="' + form_name + '"] .wpml-form-message');
                    witz_icl_message_form.html('<p style="color:#FF0000 ">ERROR</p>');
                    witz_icl_message_form.fadeIn();
                }
			}
		}
...
//ELHUYAR edition.
//Function to become special strings in special characters when html is translated
String.fromHtmlEntities = function(string) {
    return (string+"").replaceAll(/&#\d+;/gm,function(s) {
        return String.fromCharCode(s.match(/\d+/gm)[0]);
    })
};

//ELHUYAR edition.
//Function to translate and place the text of the post:
//needs a compatible language pair a non-empty string to translate
//and the type of translation (content or title) o know where to place the result
function witzultzailea_translate_call(source_lang, target_lang, text, type){
    var original_text = text;//.replaceAll(/\n+/g, '').replaceAll('"',"'");
    console.log('ORIGINAL TEXT:');
    console.log(original_text);
    var api_key = jQuery('#api_key').val();
    var api_id = jQuery('#api_id').val();
    var language_pair = source_lang + "-" + target_lang;
    var engine = "nmt";
    if(language_pair=="es-gl"||language_pair=="gl-es"||language_pair=="ca-es"||language_pair=="es-ca"||language_pair=="ca-gl"||language_pair=="gl-ca"){
        engine = "apertium";
    }
    if(language_pair=="es-an"){
        engine = "traduze";
    }
    if(language_pair=="oc_lang-fr" || language_pair=="oc_gasc-fr"){
        language_pair = "oc-fr";
        engine = "occitan";
    }
    if(language_pair=="fr-oc_gasc" || language_pair=="fr-oc_lang"){
        engine = "occitan";
    }
    if(type=="content"){
        ...
        jQuery.ajax({
            ...
            url:     icl_ajx_url,
            data:     { 
                "icl_ajx_action": "witzuli",
                "api_id" : api_id,
                "api_key" : api_key,
                "translation_engine" : engine,
                "language_pair" : language_pair,
                "content_type" : "html",
                "translation_type" : "standar",
                "text" : original_text,
                "_icl_nonce": jQuery('#_icl_nonce_sup').val(),
                //"url":'https://mt-api.elhuyar.eus/translate_string'
                "url":'https://linguatec-proxy.elia.eus/linguatec'
            },
            success:  function (msg) {
                ...
                if (msg == null){
                    ...
                    translated_text = '<b>HURRENGO ERROREA GERTATU DA:</b>'+msg.message+'\nBidalitako textuak HTML ongi eratua duela ziurtatu.';
                }
                else{
                    translated_text = htmlEntities(msg.translated_text)
                }

                translated_text =  htmlEntities(translated_text.replaceAll('&#38;','&').replaceAll('<br></br>','<br>').replaceAll('<hr></hr>','<hr>')).replaceAll("'",'"');
                if (typeof tinyMCE !== 'undefined' && (ed = tinyMCE.get('content')) && !ed.isHidden()) {
                    console.log("1")
                    ed.focus();
                    if (tinymce.isIE) {
                    ...
                    ed.execCommand('mceInsertContent', false, translated_text);
                } else if (wpml_get_block_editor()) {

                    console.log("2")
                    //wp.data.dispatch('core/editor').resetBlocks(wp.blocks.parse(translated_text));
                    wp.data.dispatch('core/block-editor').resetBlocks(wp.blocks.parse(translated_text));
                } else {
                    console.log("3")
                    wpActiveEditor = 'content';
                    edInsertContent(edCanvas, translated_text);
                }

                jQuery('#send_to_witzultzailea').next().fadeOut();
                jQuery('#send_to_witzultzailea').removeAttr('disabled');
                jQuery('#send_to_witzultzailea').next().show();
                //return
            }
        });
    }
        ...
        jQuery.ajax({
            ...
            success:  function (msg) {
                //console.log(msg)
                //console.log(msg.original_text)
                translated_text = msg.translated_text;
                message = String.fromHtmlEntities(htmlEntities(translated_text));
                if (msg == null){
                    message = text;
                }
                if (wpml_get_block_editor()) {
                    //wp.data.dispatch('core/block-editor').editPost({title: message});
                    wp.data.dispatch('core/editor').editPost({title: message});
                } else {
                    jQuery('#title-prompt-text').hide();
                    jQuery('#title').val(message);
                }
                return 
...
//ELHUYAR edition.
//Function to retrieve information about the WP post editor 
//get the text and call the translate function
function witzultzailea_translate(lang, target_lang, trid){
    jQuery('#send_to_witzultzailea').next().hide();
    jQuery('#send_to_witzultzailea').after(icl_ajxloaderimg).attr('disabled', 'disabled');
                    ...
                    success:  function (msg) {
                        if (msg.error) {
                            ...
                            try {
                                if (msg.content) {
                                    console.log("Content")
                                    witzultzailea_translate_call(lang, target_lang, msg.content, "content");
                                }
                                if (typeof msg.title !== "undefined") {
                                    console.log("Title")
                                    witzultzailea_translate_call(lang, target_lang, msg.title, "title");                                    
                                }
                                //handling of custom fields
                                //these have to be of array type with the indexes editor_type editor_name and value
                                //possible types are editor or text

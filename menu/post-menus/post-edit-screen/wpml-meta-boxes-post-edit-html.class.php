...
class WPML_Meta_Boxes_Post_Edit_HTML {
	...
	private function copy_from_original( $post ) {
		...
		if ( $source_lang && $source_lang !== $lang ) {
			...
			//ELHUYAR: edited.
			$this->display_witzultzailea_button( $source_lang, $source_lang_name, $lang, $this->sitepress->get_language_details($lang)["display_name"], $post, $trid );
	...
	//ELHUYAR: edited.
	private function display_witzultzailea_button( $source_lang, $source_lang_name,  $target_lang, $target_lang_name, $post, $trid ) {
		if( !$this->sitepress->get_setting( 'api_key' ) || !$this->sitepress->get_setting( 'api_id' ) ){
			echo '<br><a class="button-secondary" style="background: linear-gradient(to right, #00aca9, #1a3aab); color:red; white-space: normal;height: auto;min-height: 28px;line-height: normal;" type="button" href="admin.php?page=itzultzailea%2Fmenu%2Flanguages.php#lang-sec-12"><img src="' . ICL_PLUGIN_URL . '/res/img/elia_aplikazioa.jpg" alt="elhuyar logo" style="height: 14px;min-height: 24px;line-height: normal;max-width: calc(100% - 30px);">' . __('Automatic translation disabled: you need to setup and fill the api id and api key here.', 'wsitepress')  . '</a>'; 

		}
		else{


	        $url = 'https://mt-api.elhuyar.eus/proxy_language_pairs';
	        $api_id = $this->sitepress->get_setting( 'api_id' );
	        $api_key = $this->sitepress->get_setting( 'api_key' );
			...
			if(!empty($body) && isset($body[0])){
				...
				for($x = 0; $x<count($body); $x++){
					if(array_key_exists('src', $body[$x]) && array_key_exists('tgt', $body[$x])){
						array_push($allowed_pairs, $body[$x]['src'] . '-' . $body[$x]['tgt']);
					}
				}
			}
			...
			$disabled = trim( $post->post_content ) ? ' disabled' : '';
			if (!in_array(($source_lang.'-'.$target_lang), $allowed_pairs)){
				$disabled = ' disabled'; 
			}
			if (strcmp($disabled, ' disabled') == 0){
				$text = sprintf(
					esc_html__( 'Translation from %s to %s is not available', 'sitepress' ), $source_lang_name, $target_lang_name
				);
				//"' . witz_icl_PLUGIN_URL . '/res/img/elia_aplikazioa.jpg"
				wp_nonce_field( 'copy_from_original_nonce', '_icl_nonce_cfo_' . $trid );
				echo '<br><a id="send_to_witzultzailea" class="button-secondary" style="background: linear-gradient(to right, #00aca9, #1a3aab); color:white; white-space: normal;height: auto;min-height: 28px;line-height: normal;" type="button"'
				     	. $disabled . '><img src="' . ICL_PLUGIN_URL . '/res/img/elia_aplikazioa.jpg" alt="elhuyar logo" style="height: 14px;min-height: 24px;line-height: normal;max-width: calc(100% - 30px);">  '. $text .	'</a>'; 
			}
			else{
				$text = sprintf(
					esc_html__( 'Translate from %s to %s', 'sitepress' ), $source_lang_name, $target_lang_name
				);
				//"' . witz_icl_PLUGIN_URL . '/res/img/elia_aplikazioa.jpg"
				wp_nonce_field( 'copy_from_original_nonce', '_icl_nonce_cfo_' . $trid );
				echo '<br><a id="send_to_witzultzailea" class="button-secondary" style="" type="button" 
						onclick="witzultzailea_translate(\'' . esc_js( $source_lang ) . '\', \'' . esc_js( $target_lang ) . '\', \'' . esc_js( $trid ) . '\')"'
				     	. $disabled . '><img src="' . ICL_PLUGIN_URL . '/res/img/elia_aplikazioa.jpg" alt="elhuyar logo" style="height: 14px;min-height: 24px;line-height: normal;max-width: calc(100% - 30px);">  '. $text .	'</a>'; 
			}
			...
			<i class="otgs-ico-help js-otgs-popover-tooltip"
			   ...
			   title="<?php echo  esc_html__("This operation automatically translates content from the original language using elhuyar",'sitepress');?>"></i>
			   <?php
			   		echo ('<br>');
			   		echo '<input type="hidden" name="api_key" id="api_key" value="'.$this->sitepress->get_setting( 'api_key' ).'">';
			   		echo '<input type="hidden" name="api_id" id="api_id" value="'.$this->sitepress->get_setting( 'api_id' ).'">';
			   ?>
			<?php		
		}
	}

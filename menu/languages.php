...
if ( ! is_plugin_active( WPML_PLUGIN_BASENAME ) ) {
	?>
		<h2><?php esc_html_e( 'Setup Elia', 'sitepress' ); ?></h2>
...
<div class="wrap wpml-settings-container">
	...
	$compatibility_reports_args = array(
		'plugin_name'        => 'Elia',
		'plugin_uri'         => 'https://elia.eus/',
		'plugin_site'        => 'https://elia.eus/',
		'use_styles'         => true,
		'privacy_policy_url' => 'https://elia.eus/',
		'plugin_repository'  => 'elhuyar',
	);
		...
		if ( count( $active_languages ) > 1 ): ?>
				...
				<ul class="wpml-navigation-links js-wpml-navigation-links">
					...
					$navigation_items = array(
						//ELHUYAR: edited.
						//'#lang-sec-11'  => __( 'Theme and plugins reporting', 'wpml_sitepress' ),
						'#lang-sec-12'  => __( 'Elia', 'wpml_sitepress' ),
					...
					unset( $navigation_items['#wpml-translation-feedback-options'] );
			...
			<div class="wpml-section" id="lang-sec-12">
				...
				<div class="wpml-section-content">
					<div class="wpml-section-content-inner">
						<h4><?php esc_html_e('Enables machine translation using elhuyar.','sitepress')?></h4>
						<p class="explanation-text"><?php esc_html_e('Fill in the API credentials if you have them or contact them ','sitepress')?> <a href="mailto:info@elia.eus">info@elia.eus</a></p>

						<form id="witz_icl_witzultzailea_eus" name="witz_icl_witzultzailea_eus" action="">

							<?php wp_nonce_field('witz_icl_witzultzailea_eus_nonce', '_icl_nonce') ?>

							<div style="margin-bottom: 15px;">
								<label for="api_id" style="display: block; margin-bottom: 7px;">Api Id</label>
								<?php
                                    printf(
                                        '<input type="text" id="api_id" name="api_id" style="min-width: 300px;" value="' . (String) $sitepress->get_setting( 'api_id' ) .'" >'
                                    );
							    ?>
							</div>
							<div>
								<label for="api_key" style="display: block; margin-bottom: 7px;">Api Key
								</label>
							    <?php
                                    printf(
                                        '<input type="text" id="api_key" name="api_key" style="min-width: 300px;" value="' . (String) $sitepress->get_setting( 'api_key' ) .'" >'
                                    );
							    ?>
							</div>
							...
							<p class="buttons-wrap">
								<span class="wpml_icl_ajx_response" id="wpml_icl_ajx_response_ai"></span>
								<input class="button button-primary" name="save" value="<?php esc_attr_e( 'Save', 'sitepress' ) ?>" type="submit" />
							</p>
<!---
							<p class="buttons-wrap">
								<span class="wpml_icl_ajx_response" id="wpml_icl_ajx_response_hl"></span>
								<input class="button button-primary" name="save" value="<?php esc_attr_e( 'Save', 'sitepress' ) ?>" type="submit">
								--->
							</p>
						</form>
 ...
 class="active"<?php endif; ?>
		...
		<?php if ( count( $active_languages ) > 1 ): ?>
			...
			<div class="wpml-section wpml-section-redirect" id="lang-sec-9">
				...
				<div class="wpml-section-content">
					<p><?php esc_html_e( 'Elia can automatically redirect visitors according to browser language.', 'sitepress' ); ?></p>
